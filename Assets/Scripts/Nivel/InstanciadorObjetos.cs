...
public class InstanciadorObjetos : MonoBehaviour
{
    ...
    void AparecerObjeto()
    {
        int fila = Random.Range(0, 2);
        int columna = Random.Range(0, 2);
        Vector3 posicion = new Vector3(matrizCarriles[fila, columna].x, matrizCarriles[fila, columna].y, transform.position.z);
        Instantiate(asteroide, posicion, Quaternion.identity);
        Invoke("AparecerObjeto",Random.Range(0,2));
    }
}
